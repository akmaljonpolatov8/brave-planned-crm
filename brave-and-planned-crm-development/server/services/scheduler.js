...
import { sendSMS, getParentPhone } from './smsService.js';
  ...
  cron.schedule('0 9 2 * *', async () => {
    ...
    try {
      ...
      const debtors = await prisma.payment.findMany({
        ...
        include: {
          student: { select: { id: true, fullName: true, parentPhone: true, phone: true, parentName: true } },
          group: { select: { name: true } }
        }
      });

      console.log(`📊 ${debtors.length} ta qarzdor topildi (${prevMonth} oy)`);
      ...
      for (const debtor of debtors) {
        ...
        // Bu oyda allaqachon yuborilganmi tekshirish
        const existing = await prisma.smsLog.findFirst({
          where: { studentId: debtor.student.id, month: prevMonth, status: 'sent' }
        });
