...
function importWorkbook(targetPath) {
  ...
  const importData = transaction(() => {
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      const course = normalizeText(sample["Kurs"]);
      ...
      const groupName = normalizeText(sheetName || `${course} ${timeInfo.start}`);
...
try {
  ...
  const summary = importWorkbook(absolutePath);
  console.log(
    JSON.stringify(
      {
        ok: true,
