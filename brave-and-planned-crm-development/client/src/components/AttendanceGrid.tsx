            ...
            <tr key={student.id}>
              ...
              {days.map((day) => {
                ...
                return (
                  <td key={date}>
                    <button
                      className={`h-8 w-8 rounded-lg transition-all ${color} hover:brightness-125 focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none`}
                      onClick={() => onToggle(student.id, date, status)}
                      aria-label={`${student.full_name}, ${date}, holati: ${statusLabel}`}
                      title={`${student.full_name}, ${date}, holati: ${statusLabel}`}
                    />
                  </td>
                );
              })}
