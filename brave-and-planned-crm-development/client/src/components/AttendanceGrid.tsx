            ...
            <tr key={student.id}>
              ...
              {days.map((day) => {
                ...
                return (
                  <td key={date}>
                    <button
                      className={`h-8 w-8 rounded-lg ${color} transition-all hover:brightness-125 focus:outline-none focus:ring-2 focus:ring-white/50`}
                      onClick={() => onToggle(student.id, date, status)}
                      aria-label={`${student.full_name}, ${day}-kun: ${label}`}
                      title={label}
                    />
                  </td>
                );
