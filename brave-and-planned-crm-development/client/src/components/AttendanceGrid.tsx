          ...
          {students.map((student) => (
            <tr key={student.id}>
              ...
              {days.map((day) => {
                ...
                const label = status === "present" ? "keldi" : status === "absent" ? "kelmadi" : "belgilanmagan";
                return (
                  <td key={date}>
                    <button
                      aria-label={`${student.full_name}, ${date}: ${label}`}
                      title={`${student.full_name}, ${date}: ${label}`}
                      className={`h-8 w-8 rounded-lg ${color} transition-all hover:brightness-125 focus:outline-none focus:ring-2 focus:ring-brand`}
