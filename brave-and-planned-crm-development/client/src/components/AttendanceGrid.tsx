          ...
          {students.map((student) => (
            <tr key={student.id}>
              ...
              {days.map((day) => {
                ...
                const label = `${student.full_name}, ${date}: ${statusLabel}`;
                return (
                  <td key={date}>
                    <button
                      className={`h-8 w-8 rounded-lg ${color} transition-all hover:brightness-125 focus:outline-none focus:ring-2 focus:ring-[#46cfb0]`}
